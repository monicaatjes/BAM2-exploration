...
### Correlation price perception * desirability for ING specifically
...
### Correlation price perception * preference
...
trial_cor_AUSp <- trialp %>%
  ...
  correlate() %>% 
    focus(-X1, -quarter_measurement, -country, -weight, -b_value, -price_perception_value, 
          -desirability_value, -easy_value, -Love_ING, -digital_experience_value,
          -digi,-easy, -love, -preference_value, mirror= FALSE) %>%
  fashion() %>%
  dplyr::mutate(
...
cortabp <- Reduce(function(d1, d2) merge(d1, d2, by = "rowname", all.x = TRUE, all.y = FALSE),
       df_listp)
cortabp <- cortabp[-c(1,2,3,5,7,9,12,13),]
