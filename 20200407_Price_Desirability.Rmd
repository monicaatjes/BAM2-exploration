...
### Density plot desirability


```{r denplot, echo=F, eval=T, fig.width=8, fig.height=4, warning =F, message=F}
trialpr <- result %>%
  dplyr::select(X1, quarter_measurement, country, weight, b_value, price_perception_value, 
...
* This density plot shows the distribution of desirability for Australia. Other countries show equal results. 
* Relevant to be aware that the distribution is skewed to the left, with a peak score of +/-30. 
 </div> <div class="row">  
</div>
...
cortabp <- Reduce(function(d1, d2) merge(d1, d2, by = "rowname", all.x = TRUE, all.y = FALSE),
       df_listp)
cortabp <- cortabp[-c(1,2,4,5,13,15,16),]
